...
const SocialIcon = ({ icon: Icon }) => (
  <Icon className="social-icon dark:hover:text-[#00df9a]" size={30} />
);
// Footer component
const Footer = () => {
  ...
  return (
    <div className='dark:bg-[#000300] mx-auto py-16 px-4 grid lg:grid-cols-3 gap-8 dark:text-gray-300 border'>
      {/* Left section with brand and social icons */}
      <div>
        <h1 className='w-full text-3xl lg:text-4xl xl:text-5xl font-bold dark:text-[#00df9a]'>Mahendra</h1>
        <p className='py-4'>
      ...
      <div className='lg:col-span-2 flex justify-between mt-6'>
        ...
        {items.map((item, index) => (
          item.type === 'section' ? (
            <div key={index}>
              <h6 className="font-medium dark:text-gray-100 text-xl">{item.title}</h6>
              <ul>
